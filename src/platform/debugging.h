...
// ASSERT variant that only can be used on main processor that writes its message to screen / file before returning.
...
static constexpr unsigned int debugMessageMaxCount = 2048;
static CHAR16 debugMessage[debugMessageMaxCount][16384];
...
static bool foceLogToConsoleAsAddDebugMessage = false;
...
{
    ACQUIRE(debugLogLock);
    if (debugMessageCount < debugMessageMaxCount)
    {
